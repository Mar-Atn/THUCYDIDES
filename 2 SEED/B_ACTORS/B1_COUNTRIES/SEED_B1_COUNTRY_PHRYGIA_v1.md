...
|-------|---------|
| **SIM Name** | Phrygia |
| **Real-World Parallel** | Turkey |
| **Territory** | `g_phrygia` (Anatolia + Thrace), controls `g_strait_bosphorus` (Bosphorus & Dardanelles) |
| **Regime Type** | Hybrid |
...
| **Bharata** | Neutral (emerging) | Both non-aligned powers with similar hedging strategies. Defense industry cooperation (drones). Compete for "swing state" positioning. |
...
|-----------|-------|-------|
| GDP (game coins) | 11 | ~$1.1T real. 19th largest economy. Larger than it looks due to PPP — but inflation destroys real value. |
| GDP growth rate (base %) | 3.0% | Projected 3.8% for 2026. Recovery from inflation crisis. Real growth masked by currency depreciation. |
| Economic sectors | Resources: 8%, Industry: 30%, Services: 50%, Technology: 12% | Diversified. Strong defense industry (Bayraktar drones, warships). Tourism significant. |
| Tax/revenue rate | 0.25 | 25%. Revenue undermined by inflation and informal economy. |
...
|-------------------|-------|-------|
| Production costs (per unit type) | Ground: 2, Naval: 4, Tac Air: 3 | Growing domestic defense industry. Bayraktar drones are export success. Lower costs than Western producers. |
| Production capacity (max per round) | Ground: 2, Naval: 1, Tac Air: 1 | Significant and growing. Defense industry is a national priority and export earner. |
| Maintenance cost (per unit/round) | 0.30 | Moderate. Large standing army is expensive but politically useful (employment, patronage). |
| Bases hosted | None foreign (currently) | Incirlik Air Base used by Columbia historically, but access increasingly restricted. Leverage tool. |
| Bases abroad | Syria (1 ground unit) | Northeastern Syria military presence. Operations against Kurdish groups. |
| War status | Peace (with active operations) | Not formally at war. But military operations in Syria ongoing. Border security operations against Kurdish groups. Iran war fallout (missiles in Phrygia airspace). |
...
| Indicator | Value | Notes |
|-----------|-------|-------|
| Stability Index (1-10) | 5 | Hybrid regime under economic stress. Institutions weakened by personalist rule. Opposition fragmented after 2023 election defeat. Kurdish dimension creates permanent low-level instability. |
| Political Support (0-100%) | 50% | Erdogan maintains base through patronage, nationalism, and foreign policy spectacle. Economic pain eroding support. No viable succession mechanism — Erdogan IS the system. |
| Dem/Rep split | N/A | Not applicable. AKP-MHP coalition vs. fragmented opposition. |
| War tiredness | 0 | No major war. Syria operations routine. |
| Troop morale modifier | +0 (neutral) | Professional military with institutional pride. But post-2016 purges damaged officer corps. Erdogan loyalists promoted over competence in many cases. |

**Internal dynamics summary:** Erdogan's Phrygia is a one-man show. Foreign policy decisions are made by the president personally, often improvisationally. The military was purged after the 2016 coup attempt — loyal but less professionally autonomous. The opposition is fragmented and demoralized after 2023 defeat. The economy is the main vulnerability: if inflation remains catastrophic, Erdogan's patronage network frays. No succession plan exists — Erdogan's departure would create a power vacuum. The system runs on personal authority, not institutional resilience.
...
|---------------|-------|-------|
| Export restrictions in place | Expelled from F-35 program (Columbia) over S-400 purchase. Some Western tech restrictions as consequence. | Defense industry adapting through domestic development and non-Western procurement. |
| Semiconductor dependency | 20% dependent on Formosa | Low dependency. Defense industry uses less cutting-edge chips. Drone production relies on commercially available components. |
| R&D investment capacity | 1-2 coins per round | Limited by fiscal constraints. Defense R&D prioritized. Private sector tech ecosystem growing but small. |
