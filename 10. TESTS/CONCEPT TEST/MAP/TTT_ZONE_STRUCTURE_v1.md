...
| Zone ID | Display Name | Type | Owner | Adjacent Zones | Notes |
|---------|-------------|------|-------|----------------|-------|
| `g_cat_eastern` | Cathay (Eastern Seaboard) | `land_home` | `cathay` | `g_cat_western`, `g_cat_southern`, `g_sea_south_china`, `g_sea_east_china`, `g_hanguk`, `g_choson` | Industrial ruthenia. Shanghai, Beijing. Staging for Taiwan operations. |
...
### Sarmatia (Russia) -- 3 zones

| Zone ID | Display Name | Type | Owner | Adjacent Zones | Notes |
|---------|-------------|------|-------|----------------|-------|
| `g_nor_western` | Sarmatia (Western) | `land_home` | `sarmatia` | `g_nor_central`, `g_freeland`, `g_sea_baltic`, `g_sea_black` | Moscow, St. Petersburg. Borders NATO. Kaliningrad implied. Belarus corridor to Ruthenia (theater). |
| `g_nor_central` | Sarmatia (Central) | `land_home` | `sarmatia` | `g_nor_western`, `g_nor_eastern`, `g_contested_central_asia`, `g_sea_arctic` | Industrial Urals. Strategic depth. Nuclear silos. Trans-Siberian link. |
| `g_nor_eastern` | Sarmatia (Far East) | `land_home` | `sarmatia` | `g_nor_central`, `g_sea_north_pacific`, `g_choson`, `g_sea_arctic` | Pacific Fleet (Vladivostok). Borders Choson. Thin population. |
...
| `g_freeland` | Freeland | `land_home` | `freeland` | `g_teutonia`, `g_nor_western`, `g_sea_baltic` | NATO frontline. Adjacent to Sarmatia Western -- the tripwire. Eastern Europe theater border. |
...
| `g_contested_central_asia` | Central Asia | `land_contested` | `none` | `g_nor_central`, `g_cat_western`, `g_persia`, `g_bharata` | Stans (Kazakhstan, Uzbekistan, etc.). Energy resources. Cathay Belt-and-Road. Sarmatia sphere of influence. Buffer zone between three powers. |
...
| `g_sea_arctic` | Arctic | `sea` | `none` | `g_col_alaska`, `g_nor_central`, `g_nor_eastern`, `g_contested_thule`, `g_sea_giuk`, `g_sea_north_pacific` | Opening sea routes. Resource claims. Sarmatia icebreaker advantage. Climate-driven strategic competition. |
...
| `g_sea_black` | Black Sea | `sea` | `none` | `g_phrygia`, `g_nor_western` | **[CHOKEPOINT: Bosphorus]** on border with Med East (through Phrygia). Sarmatia warm-water fleet. Ruthenia access (via theater). Constrained by Phrygia chokepoint control. |
| `g_sea_baltic` | Baltic Sea | `sea` | `none` | `g_nor_western`, `g_freeland`, `g_teutonia` | Sarmatia Kaliningrad. NATO northern flank. Undersea infrastructure (cables, pipelines). Small, enclosed -- limited naval value but sabotage-relevant. |
...
| `g_sea_north_pacific` | North Pacific | `sea` | `none` | `g_col_alaska`, `g_col_hawaii`, `g_yamato`, `g_nor_eastern`, `g_sea_east_china`, `g_sea_central_pacific`, `g_sea_arctic` | Vast. Columbia-Yamato maritime corridor. Sarmatia Pacific Fleet transit. ICBM flight path. |
...
**Parent global zones:** Expands the border region between `g_nor_western`, `g_freeland`, and implies Ruthenia (which has no separate global zone -- its territory IS this theater).
...
|---------|-------------|------|---------------|----------------|-------|
| `ee_belarus` | Belarus Corridor | `land_home` | `sarmatia` | `ee_nord_border`, `ee_west_ukr`, `ee_central_ukr` | Sarmatia staging area. Threat axis to Ruthenia capital. Belarus = Sarmatia client state. NATO fears this corridor. |
| `ee_nord_border` | Sarmatia Border Region | `land_home` | `sarmatia` | `ee_belarus`, `ee_east_front_north`, `ee_east_front_central` | Russian territory. Logistics hub. Belgorod/Kursk region. Ruthenia incursions possible. |
| `ee_west_ukr` | Western Ruthenia | `land_home` | `ruthenia` | `ee_belarus`, `ee_central_ukr`, `ee_southwest`, `ee_west_border` | Lviv region. Western supply corridor. NATO border. Refugee flow origin. Relatively safe rear area. |
| `ee_central_ukr` | Central Ruthenia | `land_home` | `ruthenia` | `ee_belarus`, `ee_west_ukr`, `ee_east_front_north`, `ee_east_front_central`, `ee_capital`, `ee_south_ukr` | Geographic center. Agricultural ruthenia. Key logistics crossroads. |
| `ee_capital` | Ruthenia Capital | `land_home` | `ruthenia` | `ee_central_ukr`, `ee_south_ukr`, `ee_east_front_central` | Kyiv equivalent. Political center. Capture = regime collapse trigger. Heavily defended. Symbolic and strategic. |
| `ee_southwest` | Southwestern Ruthenia | `land_home` | `ruthenia` | `ee_west_ukr`, `ee_central_ukr`, `ee_west_border` | Odesa region. Black Sea access point. Grain export route. |
| `ee_west_border` | Western Border (NATO) | `land_home` | `ruthenia` | `ee_west_ukr`, `ee_southwest` | Freeland/NATO border. Supply entry point. If Sarmatia reaches here, NATO Article 5 question arises. Not directly attackable from Sarmatia without crossing all of Ruthenia. |
| `ee_south_ukr` | Southern Ruthenia | `land_home` | `ruthenia` | `ee_central_ukr`, `ee_capital`, `ee_east_front_south`, `ee_dnipro_line` | South-central territory. Industrial zone. Dnipro river defensive line anchor. |
| `ee_east_front_north` | Eastern Front (North) | `land_contested` | `ruthenia` | `ee_central_ukr`, `ee_nord_border`, `ee_east_front_central` | Active front line. Kharkiv axis. Heavily contested. High attrition zone. |
| `ee_east_front_central` | Eastern Front (Central) | `land_contested` | `sarmatia` | `ee_east_front_north`, `ee_nord_border`, `ee_capital`, `ee_east_front_south` | Donetsk/Luhansk. Sarmatia-controlled at start. Fortified. The "frozen" line that isn't frozen. |
| `ee_east_front_south` | Eastern Front (South) | `land_contested` | `sarmatia` | `ee_east_front_central`, `ee_south_ukr`, `ee_occupied_south` | Zaporizhzhia axis. Nuclear plant. Southern front connection. |
| `ee_dnipro_line` | Dnipro River Line | `land_home` | `ruthenia` | `ee_south_ukr`, `ee_occupied_south`, `ee_kherson` | River defensive barrier. If this falls, southern Ruthenia is exposed. Natural defense line. |
| `ee_occupied_south` | Occupied Southern Territory | `land_contested` | `sarmatia` | `ee_east_front_south`, `ee_dnipro_line`, `ee_crimea_land` | Sarmatia-occupied since 2022. Land bridge to Crimea. Contested. |
| `ee_kherson` | Kherson Region | `land_contested` | `ruthenia` | `ee_dnipro_line`, `ee_crimea_land` | West bank recaptured. Bridgehead potential. Adjacent to Crimea -- if Ruthenia pushes, Crimea is threatened. |
| `ee_crimea_naval` | Crimea & Sevastopol | `land_home` | `sarmatia` | `ee_crimea_land`, `g_sea_black` | Sarmatia Black Sea Fleet base. Annexed 2014. If lost, Sarmatia loses warm-water port. Connects to global Black Sea zone. |
...
**Starting front line:** Sarmatia holds `ee_belarus`, `ee_nord_border`, `ee_east_front_central`, `ee_east_front_south`, `ee_occupied_south`, `ee_crimea_naval` (6 zones). Ruthenia holds 8 zones. `ee_east_front_north` is Ruthenia-held but actively contested.
...
| Zone ID | Display Name | Type | Owner (start) | Adjacent Zones | Notes |
|---------|-------------|------|---------------|----------------|-------|
| `me_levantia_core` | Levantia Ruthenia | `land_home` | `levantia` | `me_levantia_north`, `g_sea_med_east` | Israel proper. Undeclared nuclear arsenal. Iron Dome/air defense concentration. Tel Aviv, Jerusalem. Small but hyper-defended. |
...
| `me_syria_lebanon` | Syria-Lebanon (Proxy Zone) | `land_contested` | `none` | `me_levantia_north`, `me_iraq_corridor`, `g_sea_med_east` | Failed state territory. Persia proxy network hub. Sarmatia air base (Tartus/Hmeimim implied). Multiple foreign forces present. No sovereign owner -- proxies contest. |
...
**Activation trigger:** Columbia military intervention against Caribe, OR Sarmatia/Cathay deploy military forces to Caribe, OR Caribe attacks Columbia interests.
...
| `gl_giuk_gap` | GIUK Gap (Theater) | `chokepoint_sea` | `none` | `gl_thule`, `gl_davis_strait`, `g_sea_giuk`, `g_sea_atlantic_east` | **[CHOKEPOINT: GIUK Gap]**. Submarine detection arrays. North Atlantic chokepoint. Sarmatia submarine transit route to Atlantic. Albion/Columbia ASW patrol. |
...
| **Blockade effects** | Sarmatia Black Sea Fleet trapped. Sarmatia loses Mediterranean access. Grain exports from Black Sea disrupted. |
...
| **Blockade minimum** | Not a traditional blockade point -- a submarine detection and denial zone. 1+ naval unit enables ASW (anti-submarine warfare) operations. Without presence, Sarmatia submarines transit freely to Atlantic. |
| **Blockade effects** | If NATO loses GIUK: Sarmatia submarines threaten Atlantic shipping lanes and Columbia's eastern seaboard. If NATO holds: Sarmatia submarine force bottled up in Arctic/Norwegian Sea. |
| **Strategic significance** | Cold War era chokepoint, newly relevant. Thule's strategic value is partly about anchoring the western end of this gap. Sarmatia submarine-launched ballistic missiles are the second-strike deterrent -- GIUK monitoring affects nuclear balance perception. |
| **Special rules** | Submarine detection: naval units in GIUK Gap can detect and engage Sarmatia submarines transiting to the Atlantic. Without GIUK presence, Sarmatia submarine deployments to `g_sea_atlantic_west` or `g_sea_atlantic_east` are undetected until they act. |
...
| **Strategic significance** | The "Monroe Doctrine" zone. Columbia's backyard. Any foreign military presence here (Sarmatia, Cathay) is a direct provocation. Low trade volume compared to Asian chokepoints but maximum political sensitivity. |
...
### Sarmatia Force Projection Routes

| Route | Path | Chokepoints | Transit Delay |
|-------|------|-------------|:-------------:|
| Sarmatia to Eastern Ereb | `g_nor_western` → theater | None (direct border) | Adjacent (instant to theater) |
| Sarmatia to Mediterranean | `g_sea_black` → `g_sea_med_east` | Bosphorus (Phrygia controls) | 1 round, IF Phrygia allows |
| Sarmatia to Atlantic | `g_nor_central`/`g_nor_eastern` → `g_sea_arctic` → `g_sea_giuk` → `g_sea_atlantic_east` | GIUK Gap | 1 round, detected if GIUK monitored |
| Sarmatia to Pacific | `g_nor_eastern` → `g_sea_north_pacific` | None | 1 round |
| Sarmatia to Mashriq | `g_nor_western` → `g_sea_black` → (Bosphorus) → `g_sea_med_east` OR overland via `g_contested_central_asia` → `g_persia` | Bosphorus OR none (overland) | 1 round either way |

> **Sarmatia's constraint:** Two of three naval egress points (Bosphorus, GIUK) are controlled or monitored by adversaries. Only the Pacific Fleet exits freely, but faces Columbia/Yamato. Sarmatia's navy is structurally trapped.
...
| Sarmatia Black Sea to Mediterranean | Depends entirely on Phrygia. Political decision, not military. Phrygia can change its mind each round. |
| Any power to Central Asia | The "Great Game" zone. Sarmatia, Cathay, and Persia all 1 round away. First mover advantage. |
...
4. **Great power home zones touch**: Sarmatia borders NATO (Freeland). Cathay borders Bharata. These adjacencies mean border tensions are spatial realities, not abstractions.
