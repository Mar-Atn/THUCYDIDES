...
- Name: meeting-popup.spec.ts >> Meeting Mode Selection Popup >> Reject invitation → no popup, no meeting
- Location: tests/meeting-popup.spec.ts:128:3
...
Locator: locator('button:has-text("Decline")').first()
Expected: visible
Timeout: 10000ms
...
Call log:
  ...
  - waiting for locator('button:has-text("Decline")').first()
